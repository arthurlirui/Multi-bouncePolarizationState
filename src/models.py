...
class RefineNet(nn.Module):
    def __init__(self, ksz=3, stride=1, dilation=1):
        super(RefineNet, self).__init__()
        self.net_in = ConvUnit(innd=3*8, outnd=64, ksz=3, stride=1, dilation=1, padding=1)
        self.conv1 = ConvUnit(innd=64, outnd=64, ksz=3, stride=1, dilation=1, padding=1)
        self.conv2 = ConvUnit(innd=64, outnd=32, ksz=3, stride=1, dilation=1, padding=1)
        self.conv3 = ConvUnit(innd=32, outnd=32, ksz=3, stride=1, dilation=1, padding=1)
        self.net_out = ConvUnit(innd=32, outnd=3, ksz=3, stride=1, dilation=1, padding=1)
    ...
    def forward(self, I_pred_phi1, I_pred_phi2, I_pred_phi3, I_pred_phi4, I_obs_phi1, I_obs_phi2, I_obs_phi3, I_obs_phi4):
        ...
        img_input = torch.cat(img_list, dim=1)
