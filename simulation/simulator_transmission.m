function S_t= simulator_transmission(filename)
%% Initial Mosic Data 
rootpath = '/home/qius0a/data/VMV_GroundTruth2018/mosic_images/color/';
% filename = 'door'; % changes 
...
for d = 8 : 0.5 : 10  % thickness of glass
    for theta_i = 1 : 15 : 90
        ...
        % multi-bounce is 3
        S_t = T2*R2*T1*S + ...
              T2*(R2^2)*T1*S*(imtranslate(S,[delta_x, 0],'FillValues',255)) + ...
              T2*(R2^4)*T1*S*(imtranslate(S,[2*delta_x, 0],'FillValues',255));    
    end
end